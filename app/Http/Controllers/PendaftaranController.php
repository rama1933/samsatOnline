...
class PendaftaranController extends Controller
{
     ...
     */
    public function indexpendaftaran1tahun()
    {
        return view('user.pendaftaran.pendaftaran1tahun.index');
    }

    public function indexpendaftaran1tahunadmin()
    {
    ...
    public function data(Request $request)
    {
        ...
        if ($user) {
            $data = $this->service->getDataPendaftaran1tahun();
        } else {
            $user_id = auth()->user()->id;
            $data = $this->service->getDataPendaftaran1tahun(user_id: $user_id);
        }
        ...
        return DataTables::of($data)
            ...
            ->addColumn('button', function ($data) use ($request) {
                return '
                 <a href="/pdf/pendaftaran1tahun/detail/' . $data->id . '"  class="btn btn-sm btn-flat btn-warning" target="_blank" title="Unduh Dokumen (PDF)"><i class="fa fa-print"></i></a>
     ...
     * @return \Illuminate\Http\Response
     */
    public function storependaftaran1tahun(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $store = $this->service->storePendafataran1tahun($request->all());
            if ($store == true) {
                return response()->json([
                    "status" => "success",
     ...
     * @return \Illuminate\Http\Response
     */
    public function showpendaftaran1tahun(Request $request)
    {
        $id = $request->id;
        $data = $this->service->getDataPendaftaran1tahun($id);
        return response()->json(
            [
                'id' => $data->id,
                'nopol' => $data->nopol,
     ...
     * @return \Illuminate\Http\Response
     */
    public function updatependaftaran1tahun(Request $request)
    {
        // dd(request()->all());
        $id = $request->id;
        $validator = Validator::make(request()->all(), []);
        ...
        if ($validator->fails()) {
            ...
            $store = $this->service->updatePendaftaran1tahun($id, $request->all());
            if ($store == true) {
                return response()->json([
                    "status" => "success",
     ...
     * @return \Illuminate\Http\Response
     */
    public function deletependaftaran1tahun(Request $request)
    {
        $id = $request->id;
        $data = $this->service->deleteDataPendaftaran1tahun($id);
        if ($data == true) {
            return response()->json([
                "status" => "success",
